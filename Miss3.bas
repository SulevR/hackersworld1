...
CLS
        ...
        INPUT "Insert Name:", nasanimi$
         IF nasanimi$ = "Father" OR nasanimi$ = "father" OR nasanimi$ = "Cleveland" OR nasanimi$ = "Cleveland" THEN GOTO 4050
         IF nasanimi$ = "Hinkd" OR nasanimi$ = "hinkd" THEN GOTO 4050
         IF nasanimi$ = "MJackson" OR nasanimi$ = "Figural" OR nasanimi$ = "figural" THEN GOTO 4050
         IF nasanimi$ = "Hinkd" OR nasanimi$ = "hinkd" THEN GOTO 4050
...
'bossi jutukas
   ...
   CLOSE #1
     ...
     IF a$ = "Raketis on v6ileib" THEN PRINT "  Ja'lle? Kohe tchekin"
      IF a$ = "Palju ]nne" OR a$ = "palju ]nne" OR a$ = "palju ]nne!" THEN GOTO 4000
      IF a$ = "Palju onne" OR a$ = "palju onne" OR a$ = "palju onne!" THEN GOTO 4000
      IF a$ = "Palju ]nne!" OR a$ = "Palju onne" OR a$ = "palju 6nne!" THEN GOTO 4000
      IF a$ = "Palju 6nne" OR a$ = "palju 6nne!" OR a$ = "palju 6nne" THEN GOTO 4000
      IF a$ = "Palju 6nne synnipaevaks" OR a$ = "Palju 6nne synnipaevaks!" THEN GOTO 4000
      IF a$ = "palju 6nne synnipaevaks" OR a$ = "palju 6nne synnipaevaks!" THEN GOTO 4000
        IF a$ = "tere" OR a$ = "Tere" OR a$ = "tere!" OR a$ = "Tere!" THEN GOTO 4010
        IF a$ = "tervist" OR a$ = "Tervist" OR a$ = "tervist!" OR a$ = "Tervist!" THEN GOTO 4010
...
3970 CLS
        ...
        PRINT TAB(24); " <Esc>  to exit"
         ...
         GOTO 1
4000     '*********** siia saab, kui soovid bossile 6nne ************
        PRINT "Aitah, aitah, 6nne on elus ikka tarvis!"
       OPEN "bostext.txt" FOR APPEND AS #1
       PRINT #1, nasanimi$, a$
       CLOSE #1
        SLEEP 1
        PRINT "NASA: Palav 6nnesoov too'tajatelt!"
        SLEEP 2: PRINT "        Sinu synnipa'eva auks ytleme nyyd k6ik: hurraa"
        SLEEP 1: PRINT "Sekretar: hurraa!"
        SLEEP 1: PRINT "Koristaja: hurraa!"
         INPUT a$
         IF a$ = "hurraa!" OR a$ = "Hurraa" OR a$ = "Hurraa!" OR a$ = "hurraa" THEN GOTO 4005
         PRINT "Sekretar: Kes Sina siin selline oled?"
         SLEEP 1: PRINT "Kohe viskan valja...": SLEEP 1
        GOTO 1
4005    PRINT "BOSS: Selle eest kohe Teile k6igile preemiat 1000$"
        SLEEP 1: PRINT "        Hurraa!"
         PRINT "Sekretar: Ja mina m6tlesin Sulle luuletuse valja:"
         SLEEP 1: PRINT "        Ta'na lendab meil rakett"
         PRINT "        taeva poole teele,"
         PRINT "        see ei olegi makett"
         PRINT "        ja r66msaks teeb ta meele!"
         PRINT : PRINT "        Yleval on kylm ja kole,"
         PRINT "        Termomeetril miinus."
         PRINT "        Soojaks aga la'heb, sest"
         PRINT "        raketi peal on ..."
         SLEEP 5: PRINT : PRINT "Aidake keegi viimane s6na riimi panna!"
         SLEEP 1: PRINT "Mis s6na see v6iks kyll olla?"
                INPUT a$
                IF a$ = "viirus" OR a$ = "Viirus" OR a$ = "viirus!" OR a$ = "Viirus!" THEN GOTO 3940
         PRINT "BOSS: Nii, nyyd sain ma vihaseks! kes see siin lollitab?"
         SLEEP 1: PRINT "       Mind Sa igatahes lolliks ei tee!!!"
      GOTO 3515
       
4010  '*****************siia saab, kui jutukas oelda tere******************
        PRINT "Teised: tere, tere, vana kere!"
        SLEEP 1: PRINT "        Oled ka piduline v6i?"
                INPUT a$
                IF a$ = "Ja" OR a$ = "ja" OR a$ = "jah" OR a$ = "Jah" THEN GOTO 4011
                PRINT "Sekretar: Mis sa siit siis otsid?"
                INPUT a$
                GOTO 3930
4011   PRINT "Sekretar: Bossile juba 6nne soovisid v6i?"
       INPUT a$
       IF a$ = "Ja" OR a$ = "ja" OR a$ = "jah" OR a$ = "Jah" THEN GOTO 4012
       PRINT "Sekretar: No mida sa siis veel ootad?"
       PRINT "          Oled ikka meie too'taja v6i?"
       INPUT a$
       IF a$ = "Ja" OR a$ = "ja" OR a$ = "jah" OR a$ = "Jah" THEN GOTO 4013
       PRINT "Sekretar: Kuidas sa idioot siis siia said...Kohe vaatan..."
       SLEEP 2: PRINT "         Igaks juhuks sulen sulle selle jutuka!"
       PRINT "CLOSED"
        DO
        LOOP UNTIL INKEY$ <> ""
       GOTO 3515
4012   PRINT "Sekretar: Kao siis juba jutukast kiiremini va'lja,"
        PRINT "         teised 6nnitlejad on jarjekorras!!!!"
                DO
                LOOP UNTIL INKEY$ <> ""
        GOTO 3515
4013  PRINT "Ytle siis salas6na!"
        INPUT "SALAS6NA:", a$
         IF a$ = "palun" OR a$ = "Palun" OR a$ = "palun!" OR a$ = "Palun!" THEN GOTO 4014
        PRINT "VALE!VALE!VALE!": PRINT "Kao va'lja siit jutukast!"
        GOTO 1
4014    PRINT "Sekretar: Noh, olgu! Ka'ivitan jutuka uuesti!"
        SLEEP 1: PRINT "         Kui sul on midagi o'elda, ytle seda kohe,kui oled"
        PRINT "         jutukasse sisse saanud!"
                DO
                LOOP UNTIL INKEY$ <> ""
        GOTO 3930
4050  INPUT "Insert User Code:", a$
